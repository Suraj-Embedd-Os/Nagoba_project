...
uint8_t error_indicator[SETUP_TOTAL_PARAMS]= {0,0,0,0,0,0,0};
...
const Setup_t setupParam[/*Page Number */] = {	
	...
	{	(uint32_t *)&cfg.set_alarm_value, 0000, 9999 },
	{ (uint32_t *)&cfg.set_alarm, 0000, 9999 },
	{	(uint32_t *)&cfg.address,1,255},
	{	(uint32_t *)&cfg.buad_rate,1200,57600},
	{	(uint32_t *)&cfg.parity,0,2},	 
};
...
//}
//		
//else if( ui_isButtonNormalPressed(BUTTON_ID_B2)) 
//{
//		if(setup_ctrl == 1)
//		{
//			menuId--;	
...
static void _save_and_exit ( uint32_t *new_cfg )
{
	...
	cfg.set_alarm_value		= (uint32_t)new_cfg[2];
	cfg.set_alarm 				= (uint32_t)new_cfg[3];
	cfg.address 					= (uint32_t)new_cfg[4];
	cfg.buad_rate					= (uint32_t)new_cfg[5];
	cfg.parity 						= (uint32_t)new_cfg[6];
	cfg.end_bit						= 0;
	ident = 1;
