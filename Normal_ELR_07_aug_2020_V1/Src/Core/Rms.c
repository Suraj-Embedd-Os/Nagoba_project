...
uint32_t __temp_write=1;
 ...
 uint32_t  Calibartion_Factor_Array[No_OF_TIME_CALL]= {500,2000,4000,6000};//,6000,8000,10000,15000,20000,30000};
/***********************************************************************************************/	
...
void Test_Conditions(void)
{
	    if(S2_Button!=true && test !=true)
			...
			else if(S1_Button!=true && test !=true)  /* REMOTE_RESET is when dg pin set all relay should be reset */
			{
